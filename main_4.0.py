...
def remove_illegal_chars(text):
    """
    移除字符串中的非法XML字符，这些字符可能导致Excel文件损坏。
    """
    if text is None:
        return ""
    # 定义非法的XML 1.0字符范围，或者使用更简单的正则表达式移除控制字符
    # 0x00 到 0x08, 0x0B, 0x0C, 0x0E 到 0x1F
    illegal_char_pattern = re.compile(
        '[\x00-\x08\x0B\x0C\x0E-\x1F\x7F]' # ASCII控制字符及DEL
        '|[\uD800-\uDFFF]' # 替代区（Surrogate halves），无效的Unicode字符
    )
    # 替换或移除非法字符
    return illegal_char_pattern.sub('', text) # 替换为空字符串

def parse_sd_params(params_str):
    """
    从Stable Diffusion的参数字符串中解析正面提示词、负面提示词和其他设置。
    params_str: 期望接收的是已经去掉换行符的生成信息字符串。
    """
    positive_prompt = ""
    negative_prompt = ""
    other_settings = ""
    ...
    # 使用 'Negative prompt:' 作为分隔符
    parts = params_str.split("Negative prompt:")
    positive_part = parts[0].strip()
    
    if len(parts) > 1:
        negative_and_settings_part = parts[1].strip()
        
        # 查找其他设置的起始点，例如"Steps:", "Sampler:"等
        # 使用更宽松的模式来匹配常见参数名称
        settings_match = re.search(r'(Steps:|Sampler:|CFG scale:|Seed:|Size:|Model hash:|Model:|Clip skip:|ENSD:|Version:|VAE:|Face restore:|Denoising strength:|Eta:|Adev:|Cfb:|Loras:|Tiling:|TI hashes:|Token merging:|Hires upscale:|Hires steps:|Hires resize:|Hires latent upscale:|Hires latent scale:|Hires upscaler:|Variation seed:|Variation strength:|AddNet Enabled:|AddNet Model:|AddNet Weight:|AddNet Strength:|AddNet Module:|AddNet Hires Ratio:|AddNet Tile Size:|AddNet Tile Padding:|AddNet Max Resolution:|AddNet Merge Method:|AddNet Save as:|AddNet Shuffle:|AddNet Denoising Start:|AddNet Denoising End:|AddNet Refiner Switch At:|AddNet Refiner Start:|AddNet Refiner End:|AddNet Skip Steps:)', negative_and_settings_part, re.IGNORECASE)
        
        if settings_match:
            negative_prompt = negative_and_settings_part[:settings_match.start()].strip()
            other_settings = negative_and_settings_part[settings_match.start():].strip()
        else:
            negative_prompt = negative_and_settings_part.strip() # 如果没有匹配到其他设置，则整个部分都是负面提示词
    
    positive_prompt = positive_part
    
    return remove_illegal_chars(positive_prompt), remove_illegal_chars(negative_prompt), remove_illegal_chars(other_settings)


def extract_image_metadata(image_path):
    """
    提取图片的Stable Diffusion生成信息。
    Args:
        image_path (str): 图片文件的绝对路径。
    Returns:
        dict: 包含以下键的字典：
            'raw_metadata': 原始Stable Diffusion元数据字符串，如果不存在则为None。
            'cleaned_metadata': 移除换行符的元数据字符串。
            'positive_prompt': 正面提示词。
            'negative_prompt': 负面提示词。
            'other_settings': 其他设置。
            'positive_prompt_word_count': 正面提示词的字数。
    """
    metadata_info = {
        "raw_metadata": None,
        "cleaned_metadata": "",
    ...
    if not os.path.exists(image_path):
        logger.warning(f"图片文件不存在，无法提取元数据: {image_path}")
        return metadata_info

    # 警告过滤器，忽略特定的UserWarning
    with warnings.catch_warnings():
        warnings.simplefilter("ignore", UserWarning) # 忽略所有UserWarning
        try:
            with Image.open(image_path) as img:
                if "parameters" in img.info: # Stable Diffusion's PNG info
                    raw_params = img.info["parameters"]
                    metadata_info["raw_metadata"] = raw_params
                    
                    # 去掉换行符的生成信息
                    cleaned_params = raw_params.replace('\n', ' ').strip()
                    metadata_info["cleaned_metadata"] = remove_illegal_chars(cleaned_params)

                    # 解析SD参数，传入 cleaned_params
                    positive_prompt, negative_prompt, other_settings = parse_sd_params(cleaned_params)
                    metadata_info["positive_prompt"] = positive_prompt
                    metadata_info["negative_prompt"] = negative_prompt
                    metadata_info["other_settings"] = other_settings
                    
                    # 计算正面提示词字数
                    if positive_prompt:
                        metadata_info["positive_prompt_word_count"] = len(positive_prompt)
                elif "exif" in img.info: # Some images store metadata in EXIF (e.g., JPEG)
                    # For JPEGs, check if there's any relevant EXIF tag, might need piexif or similar
                    # This example mainly focuses on PNG 'parameters'
                    pass
                
        except (IOError, SyntaxError) as e:
            logger.warning(f"无法读取图片文件或其元数据 (文件损坏或格式不支持): {image_path}, 错误: {e}")
        except Exception as e:
            logger.error(f"提取图片元数据时发生未知错误: {image_path}, 错误: {e}\n{traceback.format_exc()}")
...
async def _parse_card_container(card_container, image_download_queue, base_image_folder_path, target_url, current_keyword, processed_image_detail_urls):
    ...
    result_data_template = {
        ...
        "本地缩略图所在文件夹路径": "", # 新增：To be filled by download worker
    ...
    while True:
        ...
        if download_needed and image_url.startswith('http'):
            ...
            if local_filename:
                ...
                result_data_template["本地缩略图所在文件夹路径"] = os.path.dirname(abs_path) # 新增：填充文件夹路径
                ...
                # --- 新增：如果跳过下载，尝试从已存在的本地文件提取元数据 ---
                # 注意：如果文件不存在（尽管前面做了检查），这里会返回空字典
                metadata = extract_image_metadata(abs_path)
                result_data_template["stable diffusion的 ai图片的生成信息"] = metadata["raw_metadata"] if metadata["raw_metadata"] else ""
                result_data_template["去掉换行符的生成信息"] = metadata["cleaned_metadata"]
                result_data_template["正面提示词"] = metadata["positive_prompt"]
                result_data_template["负面提示词"] = metadata["negative_prompt"]
                result_data_template["其他设置"] = metadata["other_settings"]
                result_data_template["正面提示词字数"] = metadata["positive_prompt_word_count"]
...
async def md5_analyzer(md5_analysis_queue):
    ...
    while True:
        ...
        result_data_template["本地缩略图超链接"] = local_image_hyperlink
        result_data_template["本地缩略图所在文件夹路径"] = os.path.dirname(abs_local_filename) # 新增：填充文件夹路径
        result_data_template["image_md5"] = image_content_md5 # Add MD5 for consistency in results

        # --- 在这里调用元数据提取函数 ---
        metadata = extract_image_metadata(abs_local_filename)
        result_data_template["stable diffusion的 ai图片的生成信息"] = metadata["raw_metadata"] if metadata["raw_metadata"] else ""
        result_data_template["去掉换行符的生成信息"] = metadata["cleaned_metadata"]
        result_data_template["正面提示词"] = metadata["positive_prompt"]
        result_data_template["负面提示词"] = metadata["negative_prompt"]
...
async def main():
    ...
    # 更新 headers，添加 "本地缩略图所在文件夹路径" 和 SD元数据列
    headers = [
        "抓取时间", "搜索URL", "缩略图URL", "本地缩略图所在文件夹路径", 
        "本地缩略图路径", "本地缩略图超链接", "原始图片详情页链接", 
        "点赞数", "爱心数", "笑哭数", "伤心数", "打赏数", "关键词",
        "stable diffusion的 ai图片的生成信息", "去掉换行符的生成信息", 
        "正面提示词", "负面提示词", "其他设置", "正面提示词字数"
    ]
    ws.append(headers)
